...
type Filter = "All" | "Moving" | "Delayed" | "Offline";
...
function Vehicles() {
  ...
  /*
   * Load real vehicles from FastAPI
   */
  useEffect(() => {
  ...
  /*
   * Open individual vehicle
   */
  async function handleVehicleClick(vehicle: Vehicle) {
    try {
      setError("");
  ...
  const filteredVehicles = useMemo(() => {
    return vehicles.filter((vehicle) => {
      const displayStatus = getDisplayStatus(vehicle.status);

      const matchesFilter =
        filter === "All" ||
        (filter === "Moving" && displayStatus === "Moving") ||
        (filter === "Delayed" && displayStatus === "Delayed") ||
        (filter === "Offline" && displayStatus === "Offline");
      ...
      const matchesSearch =
        ...
        vehicle.cargo_type.toLowerCase().includes(searchValue);
  ...
  const moving = vehicles.filter(
    (vehicle) => getDisplayStatus(vehicle.status) === "Moving",
  ).length;

  const delayed = vehicles.filter(
    (vehicle) => getDisplayStatus(vehicle.status) === "Delayed",
  ).length;

  const offline = vehicles.filter(
    (vehicle) => getDisplayStatus(vehicle.status) === "Offline",
  ).length;
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(["All", "Moving", "Delayed", "Offline"] as Filter[]).map(
              (item) => (
                <button
                  key={item}
                  type="button"
      ...
      <div className="flex items-start gap-3 rounded-xl border border-cyan-500/10 bg-cyan-500/5 p-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">
            Vehicle information is loaded directly from the
            NEXUS-NER backend.
          </p>
