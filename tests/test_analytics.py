...
class AnalyticsEndpointTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Authenticate as CONTROL_OPERATOR because /analytics/summary
        # is protected by RBAC.
        login = cls.client.post(
            "/auth/login",
            json={
                "username": "operator",
                "password": "Operator@Nexus2026",
            },
        )

        if login.status_code != 200:
            raise RuntimeError(
                f"Could not login operator: {login.text}"
            )
    ...
    def test_02_kpis_structure(self):
        ...
        for card in kpis:
            ...
            self.assertIn(card["trend"], ("up", "down", "neutral"))
    ...
    def test_03_time_window_parameter(self):
        ...
        res_invalid2 = self.client.get(
            "/analytics/summary?days=100",
            headers=self.headers,
        )
